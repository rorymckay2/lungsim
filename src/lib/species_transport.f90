...
!##############################################################################
!
  subroutine initialise_transport(model)
 !DEC$ ATTRIBUTES DLLEXPORT,ALIAS:"SO_INITIALISE_TRANSPORT" :: INITIALISE_TRANSPORT
   ...
   !local variables
    ...
    character(len=60) :: sub_name = 'initialise_transport'
    ...
    call allocate_memory_speciestrans
     ...
     case('particles')
       print *, 'You are initialising a particle transport model'
       !initialise a mono-disperse or polydisperse locations
       call initialise_particles(0)
     end select
...
!##############################################################################
!
  subroutine solve_transport(model)
 !DEC$ ATTRIBUTES DLLEXPORT,ALIAS:"SO_INITIALISE_TRANSPORT" :: INITIALISE_TRANSPORT
    use indices
    use arrays, only: dp
   ...
   !local variables
    ...
    character(len=60) :: sub_name = 'solve_transport'
    call enter_exit(sub_name,1)
     ...
     case('particles')
       print *, 'getting implemented....'
       !Note that as V is prerequisites something needs to be added here that checks
       !these have been read in and if not sets up linear gradient based on some default parameters
       call controller_particlesolve(0)
     end select
...
!##################################################
     op_name = trim(part_param%results_location)//trim(part_param%subject)//'_particle'
  ...
  subroutine read_params_subjectinfo(lung_root,results_location,study,subject,& 
    ...
    do while (ios == 0)
       ...
       if (ios == 0) then
          ...
          select case (label)
          case ('volume')
             read(buffer, *, iostat=ios) lung_root
             print *, 'Read lung root: ', lung_root
          case ('results_location')
             read(buffer, *, iostat=ios) results_location
             print *, 'Read output location: ', results_location
          case ('study')
    ...
    do while (ios == 0)
       ...
       if (ios == 0) then
          ...
          case ('inspiration')
             ...
             print *, 'Read last breath: ', inspiration
          case default
             print *, 'Skipping invalid label at line', line
          end select
       end if
